...
class Cov(object):
    ...
    def __init__(self, data, amplicons=(), locations=(), print_progress=False, q30=False, gc=False):
        ...
        if q30 or gc:
            if bam:
                ...
                if gc:
                    self.gc = float(gc_tot) / (at_tot or 1)
        if bam: bam.close()
            ...
            for cstart, cstop, cdepth in cchrom[bisect:]:
                if cstart > entry.stop:
                    ...
                    if cdepth>=min_depth:
                        ...
                        info.range_covered.add(Entry(entry.chrom, max(entry.start, cstart), min(entry.stop, cstop), entry.name, entry.strand))
                    else:
                        info.bases_uncovered += bases
                        info.depth_uncovered += bases * cdepth
                        info.range_uncovered.add(Entry(entry.chrom, max(entry.start, cstart), min(entry.stop, cstop), entry.name, entry.strand))
                        allcovered = False
            if allcovered:
    ...
    def __init__(self, fn, chr_2_ref):
        try:
            ...
            with open(fn, "rb") as bai:
                ...
                self.bins = [defaultdict(tuple) for x in xrange(0, n_ref)]
