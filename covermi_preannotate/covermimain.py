...
import technicalreport, clinicalreport, designreport, covermiplot, tablereport
...
def covermimain(panel_path, bam_path, output_path, depth=None):
    ...
    if depth is not None:
        panel.properties.change("depth", depth)
...
def covermiargs():
    opts, args = getopt.getopt(sys.argv[1:], "", ["depth"])
    kwargs = {}
    for opt, val in opts:
        if opt == "--depth":
            kwargs["depth"] = int(val)
    covermimain(*args, **kwargs)
