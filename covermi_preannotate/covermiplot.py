...
def plot(coverage, panel, identifier, fn, transcripts=None, cumulative=False):
    if "transcripts" not in panel or not MATPLOTLIB:
        return

    with PdfPages(fn) as pdf:
        for entry in panel.transcripts.sorted_by_name:
            if transcripts is not None and entry.name not in transcripts: continue
            ...
            # amplicons
            for amplicon, y in izip(amplicons, cycle((-0.04, 0))):
