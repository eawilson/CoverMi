...
from itertools import izip, imap, islice, repeat
...
CHR_2_STR = {1: "chr1", 2: "chr2", 3: "chr3", 4: "chr4", 5: "chr5", 6: "chr6", 7: "chr7", 8: "chr8", 9: "chr9", 10: "chr10", 
...
class Entry(object):
    __slots__ = ("chrom", "start", "stop", "name", "strand")

    def __repr__(self):
        return "{}({})".format(type(self).__name__, ", ".join(self._properties()))

    def _properties(self):
        return map(lambda attr:repr(getattr(self, attr)), Entry.__slots__)
    ...
    def __init__(self, chrom, start, stop, name=DOT, strand=DOT):
        ...
        self.strand = DOT if strand==DOT else strand
...
class BaseVariant(Entry):
    ...
    def _properties(self):
        return super(BaseVariant, self)._properties() + _properties(BaseVariant, self)

    def __str__(self):
        return "{}:{} {}/{}".format(self.chrom, self.pos, self.ref, self.alt)

    def __init__(self, chrom, pos, ref, alt, strand=DOT, name=DOT):
        if ref == alt:
        ...
        super(BaseVariant, self).__init__(chrom, pos, pos + (0 if ref==MINUS else len(ref)) - 1, strand, name)
    ...
    @property
    def vartype(self):
        lenref = len(self.ref) if self.ref!=MINUS else 0
        lenalt = len(self.alt) if self.alt!=MINUS else 0
        if lenref == lenalt == 1:
            return "snp"
        elif lenref > lenalt:
            return "del"
...
class SequencedVariant(BaseVariant):
    ...
    def zygosity(self):
        ...
        if type(self).homozygous_vaf[0] <= vaf <= type(self).homozygous_vaf[1]:
            return "hemizygous" if self.chrom in (Chrom(23), Chrom(24), Chrom(25)) else "homozygous"
...
class HgmdVariant(BaseVariant):
    __slots__ = ("diseases", "weight", "hgvsc", "transcript", "gene", "disease")
...
class Variant(SequencedVariant):
    def _properties(self):
        return super(Variant, self)._properties() + ["{}={}".format(key, repr(val)) for key, val in self.__dict__.items() if not key.startswith("__")]
...
def bed(path):
    ...
    with FileContext(path, "rb") as f:
        for row in csv.reader(f, delimiter="\t"):
            yield Entry(row[CHROM], int(row[START])+1, int(row[STOP]), row[NAME] if len(row)>NAME else ".", row[STRAND] if len(row)>STRAND else ".")
            ...
            if heading_row:
                ...
                if (target_number == "1" and ontarget) or (target_number != "1" and offtarget):
                    if targets and not probes:
                        yield Entry(chrom, start+probelen[name][strand=="+"], stop-probelen[name][strand=="-"], standardised_name, strand)
                    elif targets and probes:
                        yield Entry(chrom, start, stop, standardised_name, strand)
                    elif probes and not targets:
                        yield Entry(chrom, start, start+probelen[name][strand=="+"]-1, standardised_name, strand)
                        yield Entry(chrom, stop-probelen[name][strand=="-"]+1, stop, standardised_name, strand)



def variants(path, what, genes=(), diseases=(), slots=False):
    MUTATION = 0
    GENE = 1
    DISEASE = 2
    ...
    translate = Translate()
    with FileContext(diseases, "rt") as f:
        for row in csv.reader(f, delimiter="="):
            if len(row) == 2:
                val = row[1].strip()
                if val:
    ...
    dedup = {}
    variantclass = HgmdVariant if slots else Variant
    with FileContext(path, "rb") as f:
        ...
        for chrom, pos, stop, hgvsc, strand, disease, gene, hgvsp, transcript_id in parser():
            ...
            if disease and hgvsc and "?" not in hgvsc:
                refalt = hgvsc.split(".")[-1].lstrip("0123456789_-+*()")
                if ">" in refalt:
                ...
                variant = variantclass(chrom, pos, ref, alt, name, strand)
                variant.gene = gene
...
def vcf(path, slots=False, locations=(None,)):
    ...
    def ad_depths():
        ad = map(int, row[9].split(":")[row[8].split(":").index("AD")].split(","))
        yield sum(ad)
        for d in ad[1:]:
            yield d

    def dp4_dp_depths():
        dp4 = map(int, row[7].split("DP4=")[1].split(";")[0].split(","))
        yield sum(dp4)
        yield sum(dp4[2:4])
    ...
    with FileContext(path, "rb") as f:
        ...
        for location in locations:
            ...
            while True:
                ...
                alts = row[4].split(",")
                name = row[2]
                names = repeat(name) if name == "." else name.split(",")

                for name, alt in izip(names, alts):
                    if alt not in (ref, "."):
                        if hasdepth:
                            altdepth = getdepth.next()
                            if altdepth == 0:
                                continue
...
def load_principal(path): # default dict, returns 2 if principal transcript, 1 if alternative and 0 otherwise
    ...
    if path:
        with FileContext(path, "rt") as f:
            for row in f:
                row = row.split()
                principal[row[2].split(".")[0]] = row[4].startswith("PRINCIPAL") + 1
    return principal


Targets = namedtuple("Targets", "transcript_ids gene_symbols source")
            ...
            for row in f:
                ...
                if numcols:
                    if not gene_re.match(cols[0]):
                        eprint("WARNING Invalid gene symbol: {}".format(col[0]))
                        continue
                    if numcols == 1:
                        ...
                        if not source:
                            ...
                            continue
                        needed_transcript_ids[cols[1].split(".")[0]] = cols[0]
                        needed_gene_symbols.add(cols[0])
                    else:
        ...
        for row in f:
            ...
            if source == REFSEQ:
                ...
                if what == EXONS or found is genes:
                    ...
                    for start, stop, exon in izip(row[9].rstrip(",").split(","), row[10].rstrip(",").split(","), exon_numbers):
                        start = int(start)+1
                        stop = int(stop)
...
#                            neededtranscripts[firstcol.split(".")[0]] = None
#                        else:
#                            neededgenes.add(firstcol)
#                    elif numcols == 2:
#                        neededtranscripts[cols[1].split(".")[0]] = firstcol
#                    else:
#                        eprint("WARNING Malformed targets file: {}".format(row))
...
def fasta(path, chrom, start, stop):
    if stop < start:
        raise CoverMiError("ERROR stop less than start")
    fasta = None
    regexp = re.compile(r"\.chromosome\.([0-9XYMT]+)\.")
    for fn in os.listdir(path):
        if os.path.isfile(os.path.join(path, fn)):
            match = regexp.search(fn)
            if match and Chrom(match.group(1)) == chrom:
                fasta = os.path.join(path, fn)
                break
    if not fasta:
        raise CoverMiError("ERROR No fasta found for {}".format(chrom))
    ...
    with (GzipFile if fasta.endswith(".gz") else open)(fasta, "rb") as f:
        ...
        startbyte = offset + (((start-1) // linebases) * linebytes) + ((start-1) % linebases)
        stopbyte = offset + (((stop-1) // linebases) * linebytes) + ((stop-1) % linebases)
        f.seek(startbyte)
