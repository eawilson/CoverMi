...
def load_bam_amplicons(bamfile_name, amplicons):
    cov = Coverage()

    amplicon_metrics = {}
    for chr_name in amplicons:
        for entry in amplicons[chr_name]:
            amplicon_metrics[entry[gr.NAME]] = Amplicon_info([entry[gr.NAME], "NA", "NA", gr.new(entry)])
    ...
    bedtools = _create_bedtools(bamfile_name, bedfile_temp, stranded=True)
    ...
    cov = _firstpass_into_coverage(firstpass)
    cov.amplicon_data = amplicon_metrics.values()
    cov.amplicon_data.sort(key=amplicon_number)
    return cov
...
def _firstpass_into_coverage(fp):
    cov = Coverage()
...
def perfect_coverage(gr1, perfect_depth=1000):
    firstpass = {}
    for chr_name in gr1:
        firstpass[chr_name] = []
        for entry in gr1[chr_name]:
            ...
            for pos in range(entry[gr.STOP]-read_length, entry[gr.STOP]+1):
                firstpass[chr_name].append([pos, perfect_depth])
    return _firstpass_into_coverage(firstpass)
...
class Coverage(dict):
    def __getitem__(self, chr_name):
        try:
            return super(Coverage, self).__getitem__(chr_name)
        except KeyError:
    ...
    def calculate(self, gr1, min_depth, exons=False, total=False):
        ...
        for entry in gr1.all_entries:
            ...
            if name not in resultkey:
                ...
                results.append(Coverage_info([ name, [0, 0], [0, 0], [gr.new(), gr.new()], [0, 0], entry[gr.NAME2], [0, 0] ]))
