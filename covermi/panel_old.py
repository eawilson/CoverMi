...
from .gr import Gr, bed, gff3, cosmic
...
REGEXPS = (("reference", re.compile("##gff-version 3")),
           ...
           ("variants",  re.compile("Gene name\tAccession Number\t")), #Cosmic
          )
...
class Panel(object):
    ...
    def __getitem__(self, key):
        ...
        elif key in ("alltranscripts", "allcodingregions", "allexons", "allcodingexons"):
            val = Gr(gff3(self.paths["reference"], key, names=self["names"], all_transcripts=True))
        
        else:
            raise KeyError(key)

        self.preloaded[key] = val
        return val
